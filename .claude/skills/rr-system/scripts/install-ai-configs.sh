...
main() {
  ...
  if command -v claude &>/dev/null; then
    ...
    log_step "Adding plugin marketplaces"

    # Add official claude-code-plugins marketplace
    if claude plugin marketplace add https://github.com/anthropics/claude-code-plugins 2>/dev/null; then
      log_success "  Added: claude-code-plugins marketplace"
    else
      log_warn "  claude-code-plugins marketplace may already be added"
    fi
    ...
    if claude plugin install compound-engineering 2>/dev/null; then
      log_success "  Installed: compound-engineering"
    else
      log_warn "  compound-engineering may already be installed"
    fi

    # Install commit-commands plugin from claude-code-plugins
    if claude plugin install commit-commands 2>/dev/null; then
      log_success "  Installed: commit-commands"
    else
      log_warn "  commit-commands may already be installed"
    fi

    # Install frontend-design plugin from claude-code-plugins
