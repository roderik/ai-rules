...
Comprehensive guide for implementing robust testing strategies in JavaScript/TypeScript applications using Vitest and modern testing frameworks.

## Detecting Vitest Usage

Before applying Vitest-specific patterns, check the project for Vitest indicators:

### Primary Indicators (Strong Evidence)

1. **`vitest.config.ts` or `vitest.config.js` exists** - Vitest configuration file (strongest indicator)
2. **`package.json` has `vitest` in devDependencies** - Check for `"vitest": "..."` in devDependencies
3. **`package.json` scripts use `vitest`** - Check for `"test": "vitest"`, `"test:ui": "vitest --ui"`, etc.
...
6. **CI/CD files reference vitest** - Look in `.github/workflows/*.yml`, etc.

### Detection Strategy

```bash
# Check for strong indicators
test -f vitest.config.ts && echo "Vitest detected (config file)"
test -f vitest.config.js && echo "Vitest detected (config file)"
grep -q '"vitest":' package.json && echo "Vitest detected (dependency)"
grep -q 'vitest' package.json && echo "Vitest detected (scripts)"

# Check for supporting evidence
grep -rq 'from "vitest"' src/**/*.test.ts && echo "Vitest imports found"
grep -q 'test:' vite.config.ts && echo "Vite test config found"
```

### Decision Rules

- **Strong Vitest project**: If `vitest.config.ts`/`vitest.config.js` exists OR `vitest` in devDependencies → Apply Vitest patterns
- **Likely Vitest project**: If 2+ secondary indicators → Apply Vitest patterns with caution
- **Not a Vitest project**: If using Jest (`jest.config.js`, `@types/jest`) → Use Jest patterns instead
- **Mixed project**: If both Jest and Vitest configs exist → Ask user which to use
...
describe('Type Tests', () => {
  it('should have correct return type', () => {
    ...
    expectTypeOf(getUser).returns.toEqualTypeOf<User>();
  });

  it('should accept correct parameter type', () => {
    function updateUser(id: string, updates: Partial<User>) {}

    expectTypeOf(updateUser).parameter(0).toBeString();
...
Key differences when migrating from Jest:

1. **Import syntax**: Use `import { vi } from 'vitest'` instead of global `jest`
2. **Globals**: Enable `globals: true` in config or import explicitly
3. **Config file**: `vitest.config.ts` instead of `jest.config.js`
4. **Faster execution**: Vitest uses Vite's transform pipeline
5. **ESM-first**: Better ES module support out of the box
6. **Built-in TypeScript**: No need for ts-jest
