...
Copy-pasteable **LLM ruleset extensions**. Clear, prescriptive, and phrased in the positive. Designed for generation *and* review. Keep using the “What the assistant produces” and “Checklist” sections from the base ruleset; these modules plug right in.
...
* Prefer ES modules; avoid TS “namespaces”. *If forced into legacy code, use the `namespace` keyword (not `module`) and keep it ambient‑only.*
